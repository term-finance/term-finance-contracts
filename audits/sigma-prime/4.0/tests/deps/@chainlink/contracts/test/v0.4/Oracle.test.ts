import { contract, helpers as h, matchers, oracle, setup } from "@chainlink/test-helpers";
...
describe("Oracle", () => {
  ...
  describe("#setFulfillmentPermission", () => {
    describe("when called by the owner", () => {
      beforeEach(async () => {
        await oc.connect(roles.defaultAccount).setFulfillmentPermission(roles.stranger.address, true);
      });
      ...
      it("removes an authorized node", async () => {
        await oc.connect(roles.defaultAccount).setFulfillmentPermission(roles.stranger.address, false);
        const authorized = await oc.getAuthorizationStatus(roles.stranger.address);
    ...
    describe("when called by a non-owner", () => {
      it("cannot add an authorized node", async () => {
        await matchers.evmRevert(async () => {
          await oc.connect(roles.stranger).setFulfillmentPermission(roles.stranger.address, true);
        });
      });
    });
  });
  ...
  describe("#onTokenTransfer", () => {
    ...
    describe("malicious requester", () => {
      ...
      beforeEach(async () => {
        mock = await maliciousRequesterFactory.connect(roles.defaultAccount).deploy(link.address, oc.address);
      ...
      describe("if the requester tries to create a requestId for another contract", () => {
        ...
        it("the target requester can still create valid requests", async () => {
          requester = await basicConsumerFactory.connect(roles.defaultAccount).deploy(link.address, oc.address, specId);
    ...
    it("does not allow recursive calls of onTokenTransfer", async () => {
      const requestPayload = oracle.encodeOracleRequest(specId, to, fHash, 0, "0x0");
  ...
  describe("#fulfillOracleRequest", () => {
    ...
    describe("cooperative consumer", () => {
      ...
      describe("when called by an unauthorized node", () => {
        beforeEach(async () => {
          assert.equal(false, await oc.getAuthorizationStatus(roles.stranger.address));
        });

        it("raises an error", async () => {
          await matchers.evmRevert(async () => {
            await oc.connect(roles.stranger).fulfillOracleRequest(...oracle.convertFufillParams(request, response));
          });
        });
      });
      ...
      describe("when called by an authorized node", () => {
        it("raises an error if the request ID does not exist", async () => {
          ...
          await matchers.evmRevert(async () => {
            await oc.connect(roles.oracleNode).fulfillOracleRequest(...oracle.convertFufillParams(request, response));
          });
        });
        ...
        it("does not allow a request to be fulfilled twice", async () => {
          ...
          await matchers.evmRevert(async () => {
            await oc.connect(roles.oracleNode).fulfillOracleRequest(...oracle.convertFufillParams(request, response2));
          });
    ...
    describe("with a malicious requester", () => {
      ...
      it("cannot call functions on the LINK token through callbacks", async () => {
        await matchers.evmRevert(async () => {
          await maliciousRequester.request(specId, link.address, ethers.utils.toUtf8Bytes("transfer(address,uint256)"));
        });
      });
    ...
    describe("with a malicious consumer", () => {
      ...
      describe("fails during fulfillment", () => {
        ...
        it("allows the oracle node to receive their payment", async () => {
          ...
          await oc.connect(roles.defaultAccount).withdraw(roles.oracleNode.address, paymentAmount);
      ...
      describe("tries to steal funds from node", () => {
        it("is not successful with call", async () => {
          ...
          matchers.bigNum(0, await provider.getBalance(maliciousConsumer.address));
        });
  ...
  describe("#withdraw", () => {
    describe("without reserving funds via oracleRequest", () => {
      it("does nothing", async () => {
        ...
        await matchers.evmRevert(async () => {
          await oc.connect(roles.defaultAccount).withdraw(roles.oracleNode.address, h.toWei("1"));
        });
    ...
    describe("reserving funds via oracleRequest", () => {
      ...
      beforeEach(async () => {
        ...
        const args = oracle.encodeOracleRequest(specId, mock.address, fHash, 0, "0x0");
      ...
      describe("but not freeing funds w fulfillOracleRequest", () => {
        it("does not transfer funds", async () => {
          await matchers.evmRevert(async () => {
            await oc.connect(roles.defaultAccount).withdraw(roles.oracleNode.address, payment);
          });
          const balance = await link.balanceOf(roles.oracleNode.address);
          assert.equal(0, balance.toNumber());
        });
      ...
      describe("and freeing funds", () => {
        beforeEach(async () => {
          await oc
            .connect(roles.oracleNode)
            .fulfillOracleRequest(...oracle.convertFufillParams(request, "Hello World!"));
        });
        ...
        it("does not allow input greater than the balance", async () => {
          ...
          await matchers.evmRevert(async () => {
            await oc.connect(roles.defaultAccount).withdraw(roles.stranger.address, withdrawalAmount);
          });
        ...
        it("allows transfer of partial balance by owner to specified address", async () => {
          ...
          await oc.connect(roles.defaultAccount).withdraw(roles.stranger.address, partialAmount);
        ...
        it("allows transfer of entire balance by owner to specified address", async () => {
          await oc.connect(roles.defaultAccount).withdraw(roles.stranger.address, payment);
        ...
        it("does not allow a transfer of funds by non-owner", async () => {
          await matchers.evmRevert(async () => {
            await oc.connect(roles.stranger).withdraw(roles.stranger.address, payment);
          });
          const balance = await link.balanceOf(roles.stranger.address);
  ...
  describe("#withdrawable", () => {
    ...
    beforeEach(async () => {
      ...
      assert.equal(3, receipt.logs?.length);
      request = oracle.decodeRunRequest(receipt.logs?.[2]);
      await oc.connect(roles.oracleNode).fulfillOracleRequest(...oracle.convertFufillParams(request, "Hello World!"));
    });
  ...
  describe("#cancelOracleRequest", () => {
    describe("with no pending requests", () => {
      it("fails", async () => {
        ...
        await matchers.evmRevert(async () => {
          await oc.connect(roles.stranger).cancelOracleRequest(...oracle.convertCancelParams(fakeRequest));
        });
      });
    });
    ...
    describe("with a pending request", () => {
      ...
      beforeEach(async () => {
        ...
        const args = oracle.encodeOracleRequest(specId, roles.consumer.address, fHash, 1, "0x0");
        const tx = await link.connect(roles.consumer).transferAndCall(oc.address, requestAmount, args);
        receipt = await tx.wait();
      ...
      describe("from a stranger", () => {
        it("fails", async () => {
          await matchers.evmRevert(async () => {
            await oc.connect(roles.consumer).cancelOracleRequest(...oracle.convertCancelParams(request));
          });
        });
      });
      ...
      describe("from the requester", () => {
        ...
        it("triggers a cancellation event", async () => {
          await h.increaseTime5Minutes(provider);
          const tx = await oc.connect(roles.consumer).cancelOracleRequest(...oracle.convertCancelParams(request));
          const receipt = await tx.wait();
        ...
        it("fails when called twice", async () => {
          ...
          await matchers.evmRevert(
            oc.connect(roles.consumer).cancelOracleRequest(...oracle.convertCancelParams(request)),
          );
        });
      });
    });
