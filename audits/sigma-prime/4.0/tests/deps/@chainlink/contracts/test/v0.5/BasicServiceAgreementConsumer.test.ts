...
describe("ServiceAgreementConsumer", () => {
  ...
  beforeEach(async () => {
    const meanAggregator = await meanAggregatorFactory.connect(roles.defaultAccount).deploy();
  ...
  describe("#requestEthereumPrice", () => {
    ...
    describe("#fulfillOracleRequest", () => {
      ...
      it("records the data given to it by the oracle", async () => {
        await coord.connect(roles.oracleNode).fulfillOracleRequest(request.requestId, response);
      ...
      describe("when the consumer does not recognize the request ID", () => {
        ...
        it("does not accept the data provided", async () => {
          await coord.connect(roles.oracleNode).fulfillOracleRequest(request2.requestId, response);
