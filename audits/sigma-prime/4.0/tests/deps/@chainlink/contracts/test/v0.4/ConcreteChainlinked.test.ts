...
describe("ConcreteChainlinked", () => {
  ...
  const deployment = setup.snapshot(provider, async () => {
    ...
    cc = await concreteChainlinkedFactory.connect(roles.defaultAccount).deploy(link.address, oc.address);
  });
  ...
  describe("#newRequest", () => {
    it("forwards the information to the oracle contract through the link token", async () => {
      ...
      const [jId, cbAddr, cbFId, cborData] = receipt.logs ? oracle.decodeCCRequest(receipt.logs[0]) : [];
  ...
  describe("#cancelChainlinkRequest", () => {
    ...
    beforeEach(async () => {
      ...
      ecc = await concreteChainlinkedFactory.connect(roles.defaultAccount).deploy(link.address, emptyOracle.address);
  ...
  describe("#recordChainlinkFulfillment(modifier)", () => {
    ...
    it("emits an event marking the request fulfilled", async () => {
      const tx = await oc.fulfillOracleRequest(
        ...oracle.convertFufillParams(request, ethers.utils.formatBytes32String("hi mom!")),
      );
      const { logs } = await tx.wait();
  ...
  describe("#addExternalRequest", () => {
    ...
    beforeEach(async () => {
      mock = await concreteChainlinkedFactory.connect(roles.defaultAccount).deploy(link.address, oc.address);
