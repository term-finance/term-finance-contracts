import { contract, extensions, helpers as h, setup } from "@chainlink/test-helpers";
...
describe("SchnorrSECP256K1", () => {
  ...
  it("Knows a good Schnorr signature from bad", async () => {
    ...
    async function checkSignature(signature: ethers.utils.BigNumberish): Promise<boolean> {
  ...
  it("Accepts the signatures generated on the go side", async () => {
    ...
    for (let i = 0; i < Math.min(1, f.tests.length); i++) {
      ...
      assert(
        await c.verifySignature(pX, pY.isEven() ? 0 : 1, sig, msgHash, rEIP55Address),
        "failed to verify signature constructed by golang tests",
      );
      assert(
        !(await c.verifySignature(pX, pY.isEven() ? 0 : 1, sig.add(ethers.constants.One), msgHash, rEIP55Address)),
