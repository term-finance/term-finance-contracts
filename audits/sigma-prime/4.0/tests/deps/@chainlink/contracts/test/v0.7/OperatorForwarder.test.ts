...
describe("OperatorForwarder", () => {
  ...
  const deployment = setup.snapshot(provider, async () => {
    ...
    const event = h.findEventIn(receipt, operatorForwarderDeployer.interface.events.ForwarderDeployed);
    operatorForwarder = await operatorForwarderFactory.connect(roles.defaultAccount).attach(event?.args?.[0]);
  });
  ...
  describe("#forward", () => {
    ...
    const payload = getterSetterFactory.interface.functions.setBytes.encode([bytes]);
    ...
    describe("when called by an unauthorized node", () => {
      it("reverts", async () => {
        await matchers.evmRevert(async () => {
          await operatorForwarder.connect(roles.stranger).forward(mock.address, payload);
        });
      });
    });
    ...
    describe("when called by an authorized node", () => {
      describe("when attempting to forward to the link token", () => {
        it("reverts", async () => {
          ...
          await matchers.evmRevert(async () => {
            await operatorForwarder.connect(roles.defaultAccount).forward(link.address, sighash);
          });
        });
      });
      ...
      describe("when forwarding to any other address", () => {
        it("forwards the data", async () => {
          const tx = await operatorForwarder.connect(roles.defaultAccount).forward(mock.address, payload);
          await tx.wait();
        ...
        it("perceives the message is sent by the OperatorForwarder", async () => {
          ...
          const logData = mock.interface.events.SetBytes.decode(log.data, log.topics);
          assert.equal(utils.getAddress(logData.from), operatorForwarder.address);
        });
      });
    });
  });
