...
describe("PreCoordinator", () => {
  ...
  const deployment = setup.snapshot(provider, async () => {
    ...
    pc = await preCoordinatorFactory.connect(roles.defaultAccount).deploy(link.address);

    await oc1.connect(roles.defaultAccount).setFulfillmentPermission(roles.oracleNode1.address, true);
    await oc2.connect(roles.defaultAccount).setFulfillmentPermission(roles.oracleNode2.address, true);
    await oc3.connect(roles.defaultAccount).setFulfillmentPermission(roles.oracleNode3.address, true);
    await oc4.connect(roles.defaultAccount).setFulfillmentPermission(roles.oracleNode4.address, true);
  });
  ...
  describe("#createServiceAgreement", () => {
    it("emits the NewServiceAgreement log", async () => {
      ...
      expect(h.findEventIn(receipt, pc.interface.events.NewServiceAgreement)).toBeDefined();
    });
    ...
    it("creates a service agreement", async () => {
      ...
      const receipt = await tx.wait();
      const { saId } = h.eventArgs(h.findEventIn(receipt, pc.interface.events.NewServiceAgreement));
      ...
      assert.deepEqual(sa.oracles, [oc1.address, oc2.address, oc3.address, oc4.address]);
  ...
  describe("#onTokenTransfer", () => {
    describe("when called by an address other than the LINK token", () => {
      it("reverts", async () => {
        const notLink = await linkTokenFactory.connect(roles.defaultAccount).deploy();
        ...
        const receipt = await tx.wait();
        const saId = h.eventArgs(h.findEventIn(receipt, pc.interface.events.NewServiceAgreement)).saId;

        const badRc = await requesterConsumerFactory.connect(roles.consumer).deploy(notLink.address, pc.address, saId);

        await notLink.connect(roles.defaultAccount).transfer(badRc.address, totalPayment);

        await matchers.evmRevert(badRc.connect(roles.consumer).requestEthereumPrice(currency, totalPayment, {}));
      });
    });
    ...
    describe("when called by the LINK token", () => {
      ...
      beforeEach(async () => {
        ...
        const receipt = await tx.wait();
        saId = h.eventArgs(h.findEventIn(receipt, pc.interface.events.NewServiceAgreement)).saId;

        rc = await requesterConsumerFactory.connect(roles.consumer).deploy(link.address, pc.address, saId);
        await link.transfer(rc.address, totalPayment);
      });

      it("creates Chainlink requests", async () => {
        const tx = await rc.connect(roles.consumer).requestEthereumPrice(currency, totalPayment);
        const receipt = await tx.wait();
      ...
      describe("when insufficient payment is supplied", () => {
        it("reverts", () => matchers.evmRevert(rc.connect(roles.consumer).requestEthereumPrice(currency, payment)));
      });
      ...
      describe("when the same nonce is used twice", () => {
        ...
        let args: string;
        beforeEach(async () => {
          args = oracle.encodeOracleRequest(saId, rc.address, fHash, nonce, "0x0");
          await link.transferAndCall(pc.address, totalPayment, args);
        });

        it("reverts", () => matchers.evmRevert(link.transferAndCall(pc.address, totalPayment, args)));
      });
      ...
      describe("when too much payment is supplied", () => {
        it("sends the extra back to the requester", async () => {
          ...
          await rc.connect(roles.consumer).requestEthereumPrice(currency, extraPayment);
  ...
  describe("#chainlinkCallback", () => {
    ...
    describe("when the requester and consumer are the same", () => {
      beforeEach(async () => {
        const reqTx = await rc.connect(roles.consumer).requestEthereumPrice(currency, totalPayment);
      ...
      describe("when called by a stranger", () => {
        it("reverts", () =>
          matchers.evmRevert(pc.chainlinkCallback(saId, response1), "Source must be the oracle of the request"));
      });
      ...
      describe("when called by the oracle contract", () => {
        it("records the answer", async () => {
          ...
          expect(
            receipt.events?.[0].topics.find(t => t === pc.interface.events.ChainlinkFulfilled.topic),
          ).toBeDefined();
        });
      });
    ...
    describe("when consumer is different than requester", () => {
      ...
      beforeEach(async () => {
        cc = await requesterConsumerFactory.connect(roles.consumer).deploy(link.address, pc.address, saId);
        ...
        await cc.connect(roles.consumer).addExternalRequest(pc.address, localRequestId);
      });
  ...
  describe("#withdrawLink", () => {
    ...
    describe("when called by a stranger", () => {
      it("reverts", () => matchers.evmRevert(pc.connect(roles.stranger).withdrawLink()));
    });
      ...
      it("does not allow others to call", () =>
        matchers.evmRevert(
          pc
            ...
            .cancelOracleRequest(request.requestId, request.payment, request.callbackFunc, request.expiration),
