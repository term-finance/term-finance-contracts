...
describe("GetterSetter", () => {
  ...
  describe("#setBytes32Val", () => {
    it("updates the bytes32 value", async () => {
      ...
      assert.equal(ethers.utils.toUtf8String(currentBytes32), ethers.utils.toUtf8String(bytes32));
    });
    ...
    it("logs an event", async () => {
      ...
      assert.equal(roles.stranger.address.toLowerCase(), args.from.toLowerCase());
      assert.equal(ethers.utils.toUtf8String(bytes32), ethers.utils.toUtf8String(args.value));
    });
  });
