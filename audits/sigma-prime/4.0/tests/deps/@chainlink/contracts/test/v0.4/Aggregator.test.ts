...
describe("Aggregator", () => {
  ...
  describe("#requestRateUpdate", () => {
    ...
    describe("basic updates", () => {
      ...
      it("trigger a request to the oracle and accepts a response", async () => {
        ...
        await oc1.fulfillOracleRequest(...oracle.convertFufillParams(request, response));
      ...
      it("emits a log with the response, answer ID, and sender", async () => {
        ...
        const fulfillOracleRequest = await oc1.fulfillOracleRequest(...oracle.convertFufillParams(request, response));
        ...
        assert.equal(oc1.address, ethers.utils.getAddress(receivedLog?.topics?.[3]?.slice(26, 66) ?? ""));
      });
    ...
    describe("with multiple oracles", () => {
      ...
      it("triggers requests to the oracles and the median of the responses", async () => {
        ...
        for (let i = 0; i < oracles.length; i++) {
          ...
          await o.fulfillOracleRequest(...oracle.convertFufillParams(request, responses[i]));
        }
      ...
      it("does not accept old responses", async () => {
        ...
        for (let i = 0; i < oracles.length; i++) {
          ...
          const request = oracle.decodeRunRequest(log);
          await oracles[i].fulfillOracleRequest(...oracle.convertFufillParams(request, response2));
        }
        matchers.bigNum(response2, await rate.latestAnswer());

        for (let i = 0; i < oracles.length; i++) {
          await oracles[i].fulfillOracleRequest(...oracle.convertFufillParams(requests[i], response1));
        }
  ...
  describe("#updateRequestDetails", () => {
    ...
    describe("when called by the owner", () => {
      it("changes the amout of LINK sent on a request", async () => {
        ...
        await rate.connect(personas.Carol).updateRequestDetails(uniquePayment, 1, [oc2.address], [jobId2]);
      ...
      it("can be configured to accept fewer responses than oracles", async () => {
        ...
        const response1 = h.numToBytes32(100);
        await oc1.fulfillOracleRequest(...oracle.convertFufillParams(request1, response1));
        matchers.bigNum(response1, await rate.latestAnswer());

        const response2 = h.numToBytes32(200);
        await oc2.fulfillOracleRequest(...oracle.convertFufillParams(request2, response2));
    ...
    describe("when called by a non-owner", () => {
      it("fails", async () => {
        await matchers.evmRevert(async () => {
          await rate.connect(personas.Eddy).updateRequestDetails(basePayment, 1, [oc2.address], [jobId2]);
        });
      });
    });
    ...
    describe("when called before a past answer is fulfilled", () => {
      ...
      it("accepts answers from oracles at the time the request was made", async () => {
        ...
        await oc2.fulfillOracleRequest(...oracle.convertFufillParams(request2, responses2[0]));
        await oc3.fulfillOracleRequest(...oracle.convertFufillParams(request3, responses2[1]));
  ...
  describe("#transferLINK", () => {
    ...
    describe("when called by the owner", () => {
      it("succeeds", async () => {
        await rate.connect(personas.Carol).transferLINK(personas.Carol.address, deposit);
      ...
      describe("with a number higher than the LINK balance", () => {
        it("fails", async () => {
          await matchers.evmRevert(async () => {
            await rate.connect(personas.Carol).transferLINK(personas.Carol.address, deposit.add(basePayment));
          });
    ...
    describe("when called by a non-owner", () => {
      it("fails", async () => {
        await matchers.evmRevert(async () => {
          await rate.connect(personas.Eddy).transferLINK(personas.Carol.address, deposit);
        });
  ...
  describe("#setAuthorization", () => {
    ...
    describe("when called by an authorized address", () => {
      beforeEach(async () => {
        ...
        assert.equal(true, await rate.authorizedRequesters(personas.Eddy.address));
      });
      ...
      it("can be unset", async () => {
        ...
        assert.equal(false, await rate.authorizedRequesters(personas.Eddy.address));
  ...
  describe("#cancelRequest", () => {
    ...
    describe("when a later answer has been provided", () => {
      beforeEach(async () => {
        ...
        await oc1.fulfillOracleRequest(...oracle.convertFufillParams(request2, "17"));
  ...
  describe("testing various sets of inputs", () => {
    const tests = [
      ...
      },
      {
        name: "long unordered",
        responses: [
          333121, 323453, 337654, 345363, 345363, 333456, 335477, 333323, 332352, 354648, 983260, 333856, 335468,
          376987, 333253, 388867, 337879, 333324, 338678,
        ].map(h.numToBytes32),
    ...
    beforeEach(async () => {
      rate = await aggregatorFactory.connect(defaultAccount).deploy(link.address, basePayment, 0, [], []);
      await link.transfer(rate.address, deposit);
    });
    ...
    for (const test of tests) {
      ...
      it(test.name, async () => {
        for (let i = 0; i < responses.length; i++) {
          oracles[i] = await oracleFactory.connect(defaultAccount).deploy(link.address);
