...
describe("UpdatableConsumer", () => {
  ...
  const tokenSubnode = ethers.utils.namehash(`${tokenSubdomain}.${domain}.${tld}`);
  const oracleSubdomain = "oracle";
  const oracleSubnode = ethers.utils.namehash(`${oracleSubdomain}.${domain}.${tld}`);
  ...
  const deployment = setup.snapshot(provider, async () => {
    ...
    ensResolver = await publicResolverFacotory.connect(roles.defaultAccount).deploy(ens.address);
    const ensOracleNode = ens.connect(roles.oracleNode);
    const ensResolverOracleNode = ensResolver.connect(roles.oracleNode);

    // register tld
    await ens.setSubnodeOwner(ensRoot, h.keccak(ethers.utils.toUtf8Bytes(tld)), roles.defaultAccount.address);

    // register domain
    await ens.setSubnodeOwner(tldSubnode, h.keccak(ethers.utils.toUtf8Bytes(domain)), roles.oracleNode.address);
    ...
    uc = await updatableConsumerFactory.connect(roles.defaultAccount).deploy(specId, ens.address, domainNode);
  });
  ...
  describe("#updateOracle", () => {
    describe("when the ENS resolver has been updated", () => {
      beforeEach(async () => {
        await ensResolver.connect(roles.oracleNode).setAddr(oracleSubnode, newOracleAddress);
      });

      it("updates the contract's oracle address", async () => {
        await uc.updateOracle();
        assert.equal(newOracleAddress.toLowerCase(), (await uc.getOracle()).toLowerCase());
      });
    });
  ...
  describe("#fulfillOracleRequest", () => {
    ...
    beforeEach(async () => {
      ...
      const tx = await uc.requestEthereumPrice(h.toHex(ethers.utils.toUtf8Bytes(currency)));
      const receipt = await tx.wait();
      request = oracle.decodeRunRequest(receipt.logs?.[3]);
    });

    it("records the data given to it by the oracle", async () => {
      await oc.fulfillOracleRequest(...oracle.convertFufillParams(request, response));
    ...
    describe("when the oracle address is updated before a request is fulfilled", () => {
      ...
      it("does not accept responses from the new oracle for the old requests", async () => {
        await matchers.evmRevert(async () => {
          await uc.connect(roles.oracleNode).fulfill(request.requestId, h.toHex(response));
        });
      ...
      it("still allows funds to be withdrawn from the oracle", async () => {
        ...
        matchers.bigNum(0, await link.balanceOf(uc.address), "Initial balance should be 0");
