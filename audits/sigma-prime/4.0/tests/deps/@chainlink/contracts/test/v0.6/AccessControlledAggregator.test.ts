...
describe("AccessControlledAggregator", () => {
  ...
  const deployment = setup.snapshot(provider, async () => {
    ...
    aggregator = await (aggregatorFactory as any).connect(personas.Carol).deploy(
      link.address,
      paymentAmount,
      timeout,
      emptyAddress,
  ...
  describe("#constructor", () => {
    ...
    it("sets the description", async () => {
      assert.equal(description, h.parseBytes32String(await aggregator.description()));
    });
  });
  ...
  describe("#getAnswer", () => {
    beforeEach(async () => {
      await aggregator
        ...
        .changeOracles([], [personas.Neil.address], [personas.Neil.address], minAns, maxAns, rrDelay);
    ...
    describe("when read by a contract", () => {
      describe("without explicit access", () => {
        it("reverts", async () => {
          await matchers.evmRevert(testHelper.readGetAnswer(aggregator.address, 0), "No access");
        });
      });
    ...
    describe("when read by a regular account", () => {
      ...
      describe("with access", () => {
        it("succeeds", async () => {
          await aggregator.connect(personas.Carol).addAccess(personas.Eddy.address);
          const round = await aggregator.latestRound();
          await aggregator.connect(personas.Eddy).getAnswer(round);
        });
      });
  ...
  describe("#getTimestamp", () => {
    ...
    describe("when read by a contract", () => {
      describe("without explicit access", () => {
        it("reverts", async () => {
          await matchers.evmRevert(testHelper.readGetTimestamp(aggregator.address, 0), "No access");
        });
      });
    ...
    describe("when read by a regular account", () => {
      describe("without explicit access", () => {
        it("succeeds", async () => {
          const round = await aggregator.latestRound();
          const currentTimestamp = await aggregator.connect(personas.Eddy).getTimestamp(round);
  ...
  describe("#latestAnswer", () => {
    ...
    describe("when read by a contract", () => {
      describe("without explicit access", () => {
        it("reverts", async () => {
          await matchers.evmRevert(testHelper.readLatestAnswer(aggregator.address), "No access");
        });
      });
  ...
  describe("#latestTimestamp", () => {
    ...
    describe("when read by a contract", () => {
      describe("without explicit access", () => {
        it("reverts", async () => {
          await matchers.evmRevert(testHelper.readLatestTimestamp(aggregator.address), "No access");
        });
      });
    ...
    describe("when read by a regular account", () => {
      describe("without explicit access", () => {
        it("succeeds", async () => {
          const currentTimestamp = await aggregator.connect(personas.Eddy).latestTimestamp();
          assert.isAbove(currentTimestamp.toNumber(), 0);
        });
      });
