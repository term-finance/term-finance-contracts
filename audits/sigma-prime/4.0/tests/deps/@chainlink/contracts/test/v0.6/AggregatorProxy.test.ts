  ...
  describe("#getAnswer", () => {
    ...
    describe("when the answer reverts in a non-predicted way", () => {
      it("reverts", async () => {
        ...
        await matchers.evmRevert(proxy.getAnswer(proxyId), "Raised by Reverter.sol");
      });
    });
  ...
  describe("#latestTimestamp", () => {
    ...
    describe("after being updated to another contract", () => {
      ...
      it("pulls the timestamp from the new aggregator", async () => {
        matchers.bigNum(await aggregator2.latestTimestamp(), await proxy.latestTimestamp());
        const latestRound = await proxy.latestRound();
        matchers.bigNum(await aggregator2.latestTimestamp(), await proxy.getTimestamp(latestRound));
      });
    });
  });
  ...
  describe("#proposedGetRoundData", () => {
    ...
    describe("when an aggregator has been proposed", () => {
      ...
      describe("after the aggregator has been confirmed", () => {
        ...
        it("reverts", async () => {
          const roundId = await aggregator2.latestRound();
          await matchers.evmRevert(proxy.proposedGetRoundData(roundId), "No proposed aggregator present");
        });
      });
    });
  });
  ...
  describe("#proposedLatestRoundData", () => {
    ...
    describe("when an aggregator has been proposed", () => {
      ...
      describe("after the aggregator has been confirmed", () => {
        ...
        it("reverts", async () => {
          await matchers.evmRevert(proxy.proposedLatestRoundData(), "No proposed aggregator present");
        });
      });
    });
  });
