import { contract, matchers, helpers as h, setup } from "@chainlink/test-helpers";
    ...
    beforeEach(async () => {
      await setup.snapshot(provider, async () => {
        ...
        aggregator = await (aggregatorFactory as any)
          ...
          .deploy(link.address, 0, 0, emptyAddress, 0, h.bigNum(2).pow(254), decimals, h.toBytes32String("TEST/LINK"), {
            gasLimit: 8_000_000,
          });
        proxy = await proxyFactory.connect(personas.Default).deploy(aggregator.address, emptyAddress);

        await aggregator.changeOracles([], [personas.Neil.address], [personas.Neil.address], 1, 1, 0);
