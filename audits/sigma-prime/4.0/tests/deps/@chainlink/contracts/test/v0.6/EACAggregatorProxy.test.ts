...
describe("EACAggregatorProxy", () => {
  ...
  const deployment = setup.snapshot(provider, async () => {
    ...
    aggregator = await aggregatorFactory.connect(defaultAccount).deploy(decimals, 0);
    ...
    proxy = await proxyFactory.connect(defaultAccount).deploy(aggregator.address, controller.address);
  });
  ...
  describe("callers can call view functions without explicit access", () => {
    ...
    it("#proposedGetRoundData", async () => {
      aggregator2 = await aggregatorFactory.connect(defaultAccount).deploy(decimals, answer2);
  ...
  describe("if the caller is granted access", () => {
    beforeEach(async () => {
      ...
      matchers.bigNum(ethers.utils.bigNumberify(answer), await aggregator.latestAnswer());
    ...
    it("pulls the timestamp from the aggregator", async () => {
      matchers.bigNum(await aggregator.latestTimestamp(), await proxy.latestTimestamp());
      const latestRound = await proxy.latestRound();
      matchers.bigNum(await aggregator.latestTimestamp(), await proxy.getTimestamp(latestRound));
    });
    ...
    describe("when read from a contract that is not permissioned", () => {
      ...
      it("does not allow reading", async () => {
        await matchers.evmRevert(testHelper.readLatestRoundData(proxy.address), "No access");
      });
    });
  });
  ...
  describe("#setController", () => {
    ...
    beforeEach(async () => {
      newController = await accessControlFactory.connect(defaultAccount).deploy();
    });

    describe("when called by a stranger", () => {
      it("reverts", async () => {
        await matchers.evmRevert(async () => {
          await proxy.connect(personas.Carol).setController(newController.address);
