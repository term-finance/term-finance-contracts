...
def constants():
    ...
    return types.SimpleNamespace(
        ...
        INITIALIZER_APPROVAL_ROLE=web3.keccak(text="INITIALIZER_APPROVAL_ROLE"),
...
def deploy_protocol(
    ...
    params = [treasuryAddress, protocolReserveAddress, controllerAdmin, devOps, owner]
    termController, _, _ = UUPS_proxy_deploy_and_initialize(TermController, params)

    termController.pairInitializer(termInitializer, {"from": owner})
    ...
    mockPurchaseFeed_usdc_fallback = TestPriceFeed.deploy(
        decimals,
        description,
        version,
        roundId,
    ...
    refreshRateThreshold = 60 * 60 * 24     # 24 hours (in s)
    oracle.addNewTokenPriceFeed(
        collateralToken_wbtc.address, mockCollateralFeed_wbtc.address, refreshRateThreshold, {"from": devOps}
    )
    oracle.addNewTokenPriceFeed(
        collateralToken_weth.address, mockCollateralFeed_weth.address, refreshRateThreshold, {"from": devOps}
    )
    oracle.addNewTokenPriceFeed(
        purchaseToken_usdc.address, mockPurchaseFeed_usdc.address, refreshRateThreshold, {"from": devOps}
    )
    ...
    termController.pairAuction(auction.address, {"from": termInitializer})
    ...
    termController.pairAuction(auction2.address, {"from": termInitializer})
    ...
    auction2.pairTermContracts(
        eventEmitter.address,
        termController.address,     # TODO wonder if it needs to be a separate controller?
        termRepoServicer2.address,
    ...
    contracts.update(
        {
            ...
            "mockPurchaseFeed_usdc_fallback": mockPurchaseFeed_usdc_fallback,
...
def protocol_another_auction(
    ...
    termController.pairAuction(auction3.address, {"from": termInitializer})
