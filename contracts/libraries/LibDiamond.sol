...
interface IDiamondPause {
    /// @notice Pauses the diamond contract
    function pauseDiamond() external;

    /// @notice Unpauses the diamond contract
    function unpauseDiamond() external;
...
interface IDiamondInit {
    /// @notice Initializes diamond access control roles
...
library LibDiamond {
    ...
    function replaceFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
        ...
        for (uint256 selectorIndex; selectorIndex < _functionSelectors.length; selectorIndex++) {
            bytes4 selector = _functionSelectors[selectorIndex];
            for (uint256 i = 0; i < selectorIndex; i++) {
                require(_functionSelectors[i] != selector, "Duplicate function selector");
            }
            address oldFacetAddress = ds.selectorToFacetAndPosition[selector].facetAddress;
