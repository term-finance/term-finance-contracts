...
contract TermRepoServicerHarness is TermRepoServicer {
    ...
    function totalCollateral(address borrower) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < 2; ++i) {
            totalCollateralAmount += termRepoCollateralManager
    ...
    function collateralBalance(
        ...
        return
            termRepoCollateralManager.getCollateralBalance(
                borrower,
                termRepoCollateralManager.collateralTokens(tokenNumber)
            );
    }
