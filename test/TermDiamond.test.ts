...
describe("TermDiamond Tests", () => {
  ...
  describe("TermDiamond Initialization", () => {
    it("should initialize with diamond cut facet", async () => {
      ...
      expect(await diamondCut.getAddress()).to.equal(
        await termDiamond.getAddress(),
      );
    });
    ...
    it("should set contract devops correctly", async () => {
      ...
      await expect(diamondCut.diamondCut([], ZeroAddress, "0x")).to.not.be
        .reverted;
    });
  ...
  describe("Diamond Loupe Functionality", () => {
    ...
    it("should return function selectors for a facet", async () => {
      ...
      const selectors = await loupe.facetFunctionSelectors(
  ...
  describe("Pausable Functionality", () => {
    ...
    describe("Pause Function", () => {
      it("should allow admin to pause the contract", async () => {
        const diamondPause = await ethers.getContractAt(
          "IDiamondPause",
          await termDiamond.getAddress(),
        );
        const loupe = await ethers.getContractAt(
        ...
        await expect(diamondPause.connect(admin).pauseDiamond()).to.emit(
          diamondPause,
          "DiamondPaused",
        );
    ...
    describe("Unpause Function", () => {
      ...
      it("should allow admin to unpause the contract", async () => {
        ...
        await expect(diamondPause.connect(admin).unpauseDiamond()).to.emit(
          diamondPause,
          "DiamondUnpaused",
        );
