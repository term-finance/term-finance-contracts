...
describe("TermRepoDeployerFactory Tests", () => {
  ...
  before(async () => {
    ...
    collateral2Token =
      (await TestTokenFactory.deploy()) as unknown as TestToken;
    ...
    const RevertContractFactory =
      await ethers.getContractFactory("TestRevertContract");
    revertContract =
      (await RevertContractFactory.deploy()) as unknown as TestRevertContract;
    ...
    await oracle
      ...
      .addNewTokenPriceFeed(
        ...
        await priceFeedPurchase.getAddress(),
        0,
      );
    await oracle
      .connect(devopsWallet)
      .addNewTokenPriceFeed(
        ...
        await priceFeedCollateral.getAddress(),
        0,
      );
    await oracle
      .connect(devopsWallet)
      .addNewTokenPriceFeed(
        await collateral2Token.getAddress(),
        await priceFeedCollateral2.getAddress(),
        0,
      );

    // ── Event Emitter ─────────────────────────────────────────────────────────
    const EmitterFactory = await ethers.getContractFactory(
      "TestTermEventEmitter",
    );
    ...
    mockController = await deployMock<ITermController>(
    ...
    const RolloverManagerFactory = await ethers.getContractFactory(
    ...
    const BidLockerFactory = await ethers.getContractFactory(
    ...
    const OfferLockerFactory = await ethers.getContractFactory(
  ...
  async function makeValidAuctionParams(
    termRepoId: string,
  ...
  describe("Constructor", () => {
    ...
    it("success — stores all constructor params correctly", async () => {
      ...
      expect(await factory.controller()).to.equal(
        await mockController.getAddress(),
      );
  ...
  describe("setImplementations", () => {
    ...
    it("success — stores all 5 impls and emits ImplementationsUpdated", async () => {
      ...
      expect(await factory.termRepoCollateralManagerImpl()).to.equal(
        collateralManagerImpl,
      );
      expect(await factory.termRepoLockerImpl()).to.equal(lockerImpl);
      expect(await factory.termRepoTokenImpl()).to.equal(tokenImpl);
      expect(await factory.termRepoRolloverManagerImpl()).to.equal(
        rolloverManagerImpl,
      );
  ...
  describe("pauseDeploying", () => {
    ...
    it("reverts when caller does not have ADMIN_ROLE", async () => {
      await expect(
        factory.connect(wallet1).pauseDeploying(),
      ).to.be.revertedWithCustomError(
        factory,
        "AccessControlUnauthorizedAccount",
      );
  ...
  describe("unpauseDeploying", () => {
    ...
    it("reverts when caller does not have ADMIN_ROLE", async () => {
      await expect(
        factory.connect(wallet1).unpauseDeploying(),
      ).to.be.revertedWithCustomError(
        factory,
        "AccessControlUnauthorizedAccount",
      );
  ...
  describe("deployTermRepo — validation", () => {
    it("reverts DeployingPaused when paused", async () => {
      ...
      await expect(
        factory.connect(wallet1).deployTermRepo(params),
    ...
    it("reverts MaturityTimestampInPast when maturityTimestamp <= block.timestamp", async () => {
      ...
      await expect(
        factory.connect(wallet1).deployTermRepo(params),
      ).to.be.revertedWithCustomError(factory, "MaturityTimestampInPast");
    });
    ...
    it("reverts ZeroRepurchaseWindow when repurchaseWindow = 0", async () => {
      ...
      await expect(
        factory.connect(wallet1).deployTermRepo(params),
      ).to.be.revertedWithCustomError(factory, "ZeroRepurchaseWindow");
    });
    ...
    it("reverts TokenZeroAddress when purchaseToken = address(0)", async () => {
      ...
      await expect(
        factory.connect(wallet1).deployTermRepo(params),
      ).to.be.revertedWithCustomError(factory, "TokenZeroAddress");
    });
    ...
    it("reverts InvalidTokenDecimals (catch path) when purchaseToken has no decimals() function", async () => {
      ...
      const params = await makeValidParams({
        purchaseToken: await revertContract.getAddress(),
      });
      await expect(
        factory.connect(wallet1).deployTermRepo(params),
    ...
    it("reverts InvalidTokenDecimals (>18 path) when purchaseToken has 19 decimals", async () => {
      ...
      const params = await makeValidParams({
        purchaseToken: await highDecimalsToken.getAddress(),
      });
      await expect(
        factory.connect(wallet1).deployTermRepo(params),
      ).to.be.revertedWithCustomError(factory, "InvalidTokenDecimals");
    ...
    it("reverts TokenNotSupportedByOracle when purchaseToken not registered", async () => {
      ...
      const params = await makeValidParams({
        purchaseToken: await unregisteredToken.getAddress(),
      });
      await expect(
        factory.connect(wallet1).deployTermRepo(params),
      ).to.be.revertedWithCustomError(factory, "TokenNotSupportedByOracle");
    });
    ...
    it("reverts NoCollateralTokens when collateralTokens = []", async () => {
      ...
      await expect(
        factory.connect(wallet1).deployTermRepo(params),
      ).to.be.revertedWithCustomError(factory, "NoCollateralTokens");
    });
    ...
    it("reverts DuplicateCollateralToken when two collaterals share the same address", async () => {
      ...
      await expect(
        factory.connect(wallet1).deployTermRepo(params),
      ).to.be.revertedWithCustomError(factory, "DuplicateCollateralToken");
    });
    ...
    it("reverts InvalidMaintenanceRatio when maintenanceRatio = 0", async () => {
      ...
      await expect(
        factory.connect(wallet1).deployTermRepo(params),
      ).to.be.revertedWithCustomError(factory, "InvalidMaintenanceRatio");
    });
    ...
    it("reverts InvalidInitialCollateralRatio when initialCollateralRatio = 0", async () => {
      ...
      await expect(
        factory.connect(wallet1).deployTermRepo(params),
      ).to.be.revertedWithCustomError(factory, "InvalidInitialCollateralRatio");
    });
    ...
    it("reverts InitialRatioBelowMaintenance when initialCollateralRatio < maintenanceRatio", async () => {
      ...
      await expect(
        factory.connect(wallet1).deployTermRepo(params),
      ).to.be.revertedWithCustomError(factory, "InitialRatioBelowMaintenance");
    });
    ...
    it("reverts InvalidLiquidatedDamage when liquidatedDamage = 0", async () => {
      ...
      await expect(
        factory.connect(wallet1).deployTermRepo(params),
      ).to.be.revertedWithCustomError(factory, "InvalidLiquidatedDamage");
    });
  ...
  describe("deployTermRepo — success", () => {
    it("deploys all 5 contracts, emits TermRepoDeployed, returns non-zero addresses", async () => {
      ...
      const expectedTermRepoId = ethers.solidityPackedKeccak256(
        ["string"],
        [params.termRepoId],
      );
      await expect(tx)
      ...
      const result = await (await factory
        .connect(wallet1)
        .deployTermRepo(params)
        .catch(() => null),
  ...
  describe("_deployProxy — ZeroAddress revert when no implementations set", () => {
    it("reverts ZeroAddress when deployTermRepo called before setImplementations", async () => {
      ...
      await emitter
        .connect(adminWallet)
        .pairTermFactory(await bareFactory.getAddress());
  ...
  describe("setTermAuctionImplementations", () => {
    ...
    it("success — stores all 3 impls and emits AuctionImplementationsUpdated", async () => {
      ...
      expect(await factory.termAuctionOfferLockerImpl()).to.equal(
        offerLockerImpl,
      );
  ...
  describe("pauseAuctionDeploys", () => {
    ...
    it("reverts when caller does not have ADMIN_ROLE", async () => {
      await expect(
        factory.connect(wallet1).pauseAuctionDeploys(),
      ).to.be.revertedWithCustomError(
        factory,
        "AccessControlUnauthorizedAccount",
      );
  ...
  describe("unpauseAuctionDeploys", () => {
    ...
    it("reverts when caller does not have ADMIN_ROLE", async () => {
      await expect(
        factory.connect(wallet1).unpauseAuctionDeploys(),
      ).to.be.revertedWithCustomError(
        factory,
        "AccessControlUnauthorizedAccount",
      );
  ...
  describe("deployAuctionAndReopenTerm — validation", () => {
    ...
    beforeEach(async () => {
      ...
      const staticResult = await factory
        .connect(wallet1)
        .deployTermRepo.staticCall(repoParams);
    ...
    it("reverts DeployingPaused when deployingPaused = true", async () => {
      ...
      await expect(
        factory
          .connect(wallet1)
          .deployAuctionAndReopenTerm(auctionParams, deployedServicerAddr),
      ).to.be.revertedWithCustomError(factory, "DeployingPaused");
    });
    ...
    it("reverts InvalidClearingPricePostProcessingOffset when offset = 2", async () => {
      ...
      await expect(
        ...
        factory,
        "InvalidClearingPricePostProcessingOffset",
      );
    });
  });
  ...
  describe("deployAuctionAndReopenTerm — success", () => {
    it("deploys 3 auction contracts, emits TermAuctionDeployed, returns non-zero addresses", async () => {
      const REPO_TERM_REPO_ID = "term-repo-001";
      const repoParams = await makeValidParams();
      const repoStaticResult = await factory
        .connect(wallet1)
        .deployTermRepo.staticCall(repoParams);
      await factory.connect(wallet1).deployTermRepo(repoParams);
      ...
      const expectedTermAuctionId = ethers.solidityPackedKeccak256(
        ["string"],
        ["term-auction-001"],
      );
      ...
      const staticResult = await factory
        .connect(wallet1)
        .deployAuctionAndReopenTerm.staticCall(
          auctionParams,
          deployedServicerAddr,
        );
      expect(staticResult.termAuction).to.not.equal(ethers.ZeroAddress);
      expect(staticResult.termAuctionBidLocker).to.not.equal(
        ethers.ZeroAddress,
      );
      expect(staticResult.termAuctionOfferLocker).to.not.equal(
        ethers.ZeroAddress,
      );
  ...
  describe("deployAuctionAndReopenTerm — ZeroAddress revert when no auction impls set", () => {
    it("reverts ZeroAddress when deployAuctionAndReopenTerm called before setTermAuctionImplementations", async () => {
      ...
      const repoParams = await makeValidParams();
      const repoStaticResult = await bareFactory
        .connect(wallet1)
        .deployTermRepo.staticCall(repoParams);
